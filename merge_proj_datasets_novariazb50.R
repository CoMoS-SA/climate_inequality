...
new_all<- read_csv("data_revision/proj_data_variaz_agriemp_bound_NOERROR_novariazb50.csv")
...
#trend<- read_csv("data_revision/proj_just_trends_v2.csv")#with outliers
trend<- read_csv("data_revision/proj_just_trends_novariazb50.csv")
trend$country[is.na(trend$country)]<-"NA" #to include Nambia
#new_ssp3<- read_csv("data_revision/comb_ssp_variaz_all_NOERROR_novariazb50.csv")
#new_ssp3$country[is.na(new_ssp3$country)]<-"NA" #to include Nambia
#new_ssp5<- read_csv("data_revision/comb_SSP5_variaz_all_NOERROR_novariazb50.csv")
...
merge5<-left_join(merge2, only_ssp3_to_join, by=c("country"="country_iso2"))
data_scenarios_projections<- left_join(merge5, only_ssp5_to_join, by=c("country"="country_iso2"))

write_csv(data_scenarios_projections, "data_revision/data_scenarios_projections_withSSP_novariazb50.csv")
...
#### check trend sign ####
check<-d2010%>%distinct(country,.keep_all=T)

world_check <- left_join(world, d2010, by=c("iso_a2"="country"))
world_check%<>%mutate(prova=case_when(
  b50_diff>0 ~ 1,
  b50_diff<0 ~ 2
))
