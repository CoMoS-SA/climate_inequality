setwd("~/Documents/replication/replication_clim_ineq_sub2")
...
data_just_trends%<>%filter(country!="SY")%>%filter(country!="BH")%>%filter(country!="ME") %>%filter(country!="MT") %>%filter(country!="SC")  #because nan in climate projections (to have comparable set of countries)
...
#write_csv(data_just_trends, "data_revision/proj_just_trends_novariazb50.csv")
...
#ginis_all<- read_csv("data_revision/ginis_all.csv")
