...
#If equal to T, alternative smoothing (linear) #Results are robust
lin.diff<-F
...
data_b%<>% filter(year>1990)%>%filter(year<2011)%>% group_by(country) %>% mutate( #to be comparable to new baseline projections
...
if(lin.diff){
  bottom_proj_wb %<>% mutate(
    comp_growth_prec = (proj_tot_prec-avg_prec)/70, 
    comp_growth_temp = (proj_mean_temp-avg_temp)/70 
  )   
}
...
bottom_proj_wb %<>% filter(year>1990) %>% group_by(country) %>% mutate( #1989 in old version (but to have exact initial gini for global inequality impacts)
...
#d2010 %<>% select(country, country_iso3, UDel_precip_popweight, UDel_precip_popweight_2, UDel_temp_popweight, temp_sq, comp_growth_prec, comp_growth_temp, agri_top20, agri_b80, bottom50, mean_prec, mean_temp, mean_diff, mean_b50, mean_growth, mean_gdp, agri_emp, avg_agri_emp, Model)
d2010 %<>% select(country, country_iso3, UDel_precip_popweight, UDel_precip_popweight_2, UDel_temp_popweight, temp_sq, comp_growth_prec, comp_growth_temp, agri_top20, agri_b80, bottom50, mean_prec, mean_temp, mean_diff, mean_b50, mean_growth, mean_gdp, avg_agri_emp, Model)
...
if(lin.diff){
  ...
  for (i in 1:length(d2010$country)){
    for (t in 1:70){
      if(output[t][!is.na(output[t])]){
        output[t+1,i] = output[t,i]+d2010$comp_growth_prec[i]}
      else{
        output[t+1,i] = NA}
    }}  
}
...
for (i in 1:length(d2010$country)){
  for (t in 1:70){
    if(output_b[t][!is.na(output_b[t])]){
      output_b[t+1,i] = output_b[t,i]+ ((-0.0331 + 2*0.0133*(output[t+1,i])) + (0.000774*d2010$agri_emp[i]) + (2*(-0.000311)*d2010$agri_emp[i]*(output[t+1,i])))*(output[t+1,i]-output[t,i])} 
    else{
      output_b[t+1,i] = NA}
  }}
...
for (i in 1:length(d2010$country)){
  for (t in 1:70){
    if(output_gdp[t][!is.na(output_gdp[t])]){
      output_gdp[t+1,i] = output_gdp[t,i] * (1+ (((0*output[t+1,i]) + (0*(output[t+1,i])^2) + (0.00236*d2010$agri_emp[i]*output[t+1,i]) + (-0.000916*d2010$agri_emp[i]*(output[t+1,i])^2)) - ((0*output[1,i]) + (0*(output[1,i])^2) + (0.00236*d2010$agri_emp[i]*output[1,i]) + (-0.000916*d2010$agri_emp[i]*(output[1,i])^2))))} 
    else{
      output_gdp[t+1,i] = NA}
  }}
...
for (i in 1:length(d2010$country)){
  for (t in 1:70){
    if(output_gdp_t[t][!is.na(output_gdp_t[t])]){
      output_gdp_t[t+1,i] = output_gdp_t[t,i] * (1+ ((0.0157*(output_t[t+1,i])-0.000759*(output_t[t+1,i])^2) - ((0.0157*(output_t[1,i])-0.000759*(output_t[1,i])^2))))}
    else{
...
long_data_proj%<>%  mutate(tot_eff_b50=grate_b50proj_prec,
                           tot_eff_gdp=grate_gdpproj_prec+grate_gdpproj_temp)
...
#write_csv(short_data_proj, "data_revision/only_clim_comparable.csv")
#write_csv(short_data_proj, "proj_data_adjusted.csv")

#write_csv(short_data_proj, "data_revision/proj_data_old_1990_TZ.csv")
...
#WHEN SSP GINIS, WE HAVE TO CHECK THAT THESE COUNTRIES ARE EXCLUDED "AE" "AL" "AO" "ME" "QA" "SC" IN ORDER TO HAVE SAME INITIAL GINIS
long_data_proj%<>%filter(substr(country,1,2)!="AE") %>%filter(substr(country,1,2)!="AL")%>%filter(substr(country,1,2)!="AO")%>%
                  filter(substr(country,1,2)!="ME")%>%filter(substr(country,1,2)!="QA")%>%filter(substr(country,1,2)!="SC")
...
if(top20_ineq==F){
  #ginis_all<-read_csv("data_revision/ginis_all_ssp5.csv")
  ...
  gini_for_fig<-as.data.frame(gini_for_fig)%>%mutate(Scenario="Only climate no outliers - Global - Prec")
  ginis_all<-gini_for_fig }

if(top20_ineq==T){
  gini_for_fig<-cbind(ginis$initial[1], ginis$mean_final[1], ginis$se_fin[1])
  colnames(gini_for_fig)<-c("Gini initial", "Gini mean final", "SE")
  gini_for_fig<-as.data.frame(gini_for_fig)%>%mutate(Scenario="Only climate no outliers - Top20 - Prec")
...
if(top20_ineq==F){
  gini_for_fig<-cbind(ginis$initial[1], ginis$mean_final[1], ginis$se_fin[1])
  colnames(gini_for_fig)<-c("Gini initial", "Gini mean final", "SE")
  gini_for_fig<-as.data.frame(gini_for_fig)%>%mutate(Scenario="Only climate no outliers - Global - Prec+Temp")
...
if (top20_ineq==T){
  gini_for_fig<-cbind(ginis$initial[1], ginis$mean_final[1], ginis$se_fin[1])
  colnames(gini_for_fig)<-c("Gini initial", "Gini mean final", "SE")
  gini_for_fig<-as.data.frame(gini_for_fig)%>%mutate(Scenario="Only climate no outliers - Top20 - Prec+Temp")
  
  ginis_all<-rbind(ginis_all,gini_for_fig)}


write_csv(ginis_all, paste0("data_revision/ginis_all_ssp_",ssps[i_ssp],"_rcp_",rcps[i_rcp],".csv"))
