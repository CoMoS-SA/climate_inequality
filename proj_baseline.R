  ...
  for (t in 1:70){
    if(output_b[t][!is.na(output_b[t])]){
      if(!is.na(d2010$agri_emp[i]+(t*d2010$agri_diff[i])) & d2010$agri_emp[i]+(t*d2010$agri_diff[i]) > 99){
        ...
        output_b[t+1,i] = output_b[t,i]+  ((-0.0331 + 2*0.0133*(output[t+1,i])) + (0.000774*(d2010$agri_emp[i]+(t*d2010$agri_diff[i]))) + (2*(-0.000311)*(d2010$agri_emp[i]+(t*d2010$agri_diff[i]))*(output[t+1,i])))*(output[t+1,i]-output[t,i])
      }} 
    else{
      output_b[t+1,i] = NA}
  }}
...
#write_csv(short_data_proj, "data_revision/proj_data_variaz_agriemp_bound_NOERROR_novariazb50.csv")
#write_csv(output_long, "data_revision/longdataproj_NOERROR_novariazb50.csv")
...
#write_csv(ginis_all, "data_revision/ginis_all_novariazb50.csv")
